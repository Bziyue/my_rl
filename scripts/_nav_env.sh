...
ENV_NAME="${ENV_NAME:-env_isaacsim}"
CONDA_ENV_DIR="${CONDA_ENV_DIR:-/home/zdp/anaconda3/envs/${ENV_NAME}}"
ISAACSIM_DIR="${ISAACSIM_DIR:-/home/zdp/isaacsim}"
...
check_nav_runtime_prereqs() {
    ...
    if [[ ! -d "${ISAACSIM_DIR}" ]]; then
        echo "Missing Isaac Sim directory: ${ISAACSIM_DIR}" >&2
        return 1
    fi
}
...
setup_nav_runtime_env() {
    ...
    if [[ -f "${ISAACSIM_DIR}/setup_conda_env.sh" ]]; then
        set +u
    ...
    # Isaac Sim's setup script does not add omni.client on this machine.
